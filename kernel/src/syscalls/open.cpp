                           ...
                           int flags, mode_t mode)
{
	...
	if (pathname == "stdout") {
		m_files[fd] = FileManager::open(FileManager::SpecialFile::Stdout);
		return fd;
	}
	...
	m_files[fd] = FileManager::open(pathname, flags);
	return fd;
}
...
int Process::do_sys_close(int fd) {
	if (!m_files.count(fd))
		return -EBADF;
	FileDescription* description = m_files[fd];
	m_files.erase({fd, description});
	description->unref();
	return 0;
}